...
def analyse_data(path):
  for file in os.listdir(path):
      if file.endswith(".tsv") and "-ans" in file:
          ...
          grouped_data['cosine_similarity'] = data.groupby('verbosity_control')['cosine_similarity'].apply(list).reset_index()['cosine_similarity']

          # Extract F1 scores (fmeasure) from the ROUGE SCORE strings
          def extract_f1_scores(rouge_scores_list):
              rouges_f1_scores = {rougeN: [] for rougeN in rouges}
              for rouge_scores in rouge_scores_list:
                  rouge_scores_dict = eval(rouge_scores) 
                  for rougeN in rouges:
                      if rougeN in rouge_scores_dict:
                          rouges_f1_scores[rougeN].append(rouge_scores_dict[rougeN]['f1'])

              return rouges_f1_scores

          rogue_f1_scores = grouped_data['rouge_scores'].apply(extract_f1_scores)
          for rougeN in rouges:
...
def display_graphs_similarity(path):
    df = pd.read_csv(path)
    ...
    for i, score in enumerate(scores):
        ...
        for control in verbosity_controls.unique():
            subset = df[df['verbosity_control'] == control][score].dropna()
...
if __name__ == "__main__":
  ...
  # analyse_data(path)
  # display_graphs_rogue(path+"analysis.csv")
  display_graphs_similarity(path+"analysis.csv")